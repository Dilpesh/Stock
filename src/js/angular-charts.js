    ...
    function ($templateCache, $compile, $rootElement, $window, $timeout, $sce) {
        ...
        function link(scope, element, attrs) {
            ...
            function setContainers() {
                ...
                height -= getChildrenByClassname(childrens, 'ac-title')[0].clientHeight - 30;
            }
            /**
            ...
            function areaChart() {
                ...
                var max = d3.max(points, function (d) {
                    return +d.x;
                });
                var min = d3.min(points, function (d) {
                    return +d.x;
                });
                var xmm = d3.time.scale()
                ...
                var svg = d3.select(chartContainer[0]).append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom + 20).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
                var padding = d3.max(yData) * 0.2;
                y.domain([
                    d3.min(yData),
                    d3.max(yData) + padding
                ]);
                svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + height + ')').call(xAxismm);
                svg.append("text").attr("transform", "translate(" + (width / 2) + " ," + (height + margin.bottom + 10) + ")")
